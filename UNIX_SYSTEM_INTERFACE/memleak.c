...
{
    size_t *a ;                    /* Integer variable */
    size_t *b ;                    /* Integer array */
    a = Calloc(1,sizeof(int)) ;
    b = Calloc(10,sizeof(int)) ;
    printf("Calloc for integer type :- %p | %zu\n", a , *a) ;
    printf("Calloc for integer array :-%p | %zu", b , *b) ;
    return 0 ;
}
...
size_t *Calloc( size_t n_memb , size_t size )
{
/* Checks for integer overflow for multiplication for bytes to be 
   allocated(size*n_memb) */
    size_t bit_alloc = n_memb * size ;
...
/* Initialising Variables */
    size_t *free_mem ;
    if( (free_mem = malloc( bit_alloc )) != NULL ){
        for( size_t *trv_ptr=free_mem ; trv_ptr < free_mem+bit_alloc ; trv_ptr++ ){
            *trv_ptr = 0 ;
        }
    }
